...
public class Message {
    ...
    public static Multi<Message> findAll(PgPool client) {
        return client.query("SELECT * FROM chats ORDER BY text ASC").execute()
    ...
    public static Uni<Message> findByName(PgPool client, String text) {
        return client.preparedQuery("SELECT * FROM chats WHERE text = $1").execute(Tuple.of(text))
                .onItem().apply(RowSet::iterator)
                .onItem().apply(iterator -> iterator.hasNext() ? from(iterator.next()) : null);
    }

    public Uni<Long> save(PgPool client) {
        return client.preparedQuery("INSERT INTO chats (text, sender, chatid, date, chatdata) VALUES ($1, $2, $3, $4, $5) RETURNING (id)").execute(Tuple.of(text,sender, chatid, date, chatdata ))
                .onItem().apply(pgRowSet -> pgRowSet.iterator().next().getLong("id"));
    }

    public Uni<Boolean> update(PgPool client) {
        return client.preparedQuery("UPDATE chats SET text = $1, sender = $2, chatid = $3, date = $4, chatdata = $5 WHERE text = $1").execute(Tuple.of(text, sender, chatid, date,  chatdata ))
